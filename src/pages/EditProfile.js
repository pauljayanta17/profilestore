...
function EditProfile(props) {
  const [name, setname] = new useState(props.data.name);
  const [email, setemail] = new useState(props.data.email);
  const [address, setaddress] = new useState(props.data.address);
  const [city, setcity] = new useState(props.data.city);
  const [pincode, setpincode] = new useState(props.data.pincode);
  const [github, setgithub] = new useState(props.data.github);
  const [linkedin, setlinkedin] = new useState(props.data.linkedin);
  const [facebook, setfacebook] = new useState(props.data.facebook);
  const [twitter, settwitter] = new useState(props.data.twitter);
  const [projectsLinks, setprojectsLinks] = new useState(
    ...
    if (
      ...
      try {
        const resposne = await fetch(
          `https://profilestore.herokuapp.com/profile/updateprofile/${props.data._id}`,
          {
            headers: {
  ...
  return (
    <>
      <i
        ...
        data-bs-target={`#updateprofile${props.data._id}modal`}
        style={{ cursor: "pointer" }}
      ></i>

      <div
        className="modal fade modal-xl"
        id={`updateprofile${props.data._id}modal`}
        tabIndex="-1"
